...
library(ggpubr)
library(tidyr)
table_one <- totalhisty_no_missings_long %>% select(Age,truesex,race_ethnicity,kbi_y_trans_id,demo_prnt_ed_v2_2yr_l,eventname)
table_one <- table_one %>% filter(table_one$eventname == "1_year_follow_up_y_arm_1")

table_two <- AHHHH %>% select(race_ethnicity_1,race_ethnicity_2,race_ethnicity_3,race_ethnicity_4,src_subject_id)
table_two$race_ethnicity_2 <- table_two$race_ethnicity_1

table_one <- historya4 %>% select(Age,truesex,race_ethnicity,kbi_y_trans_id,demo_prnt_ed_v2_2yr_l, demo_prnt_ed_v2_l,eventname)
...
table_one %>% tbl_summary(label = list(Age ~"Patient Age", truesex~"Sex Assigned at Birth", kbi_y_trans_id~"Are you Transgender?")) %>% modify_header(label = "**4th Year Follow up**")
table_three <- historya3[historya3$Gender_Dissonance==0,] %>% select(truesex,race_ethnicity,kbi_y_trans_id)


table_three$truesex[table_three$truesex==1]<-"Male"
table_three$truesex[table_three$truesex==2]<-"Female"
table_three$truesex[table_three$truesex==3]<-"Other"

table_three$kbi_y_trans_id[table_three$kbi_y_trans_id==1] <- "Yes"
table_three$kbi_y_trans_id[table_three$kbi_y_trans_id==2] <- "Maybe"
table_three$kbi_y_trans_id[table_three$kbi_y_trans_id==3] <- "No"
table_three$kbi_y_trans_id[table_three$kbi_y_trans_id==4] <- "I Don't Understand"
table_three$kbi_y_trans_id[table_three$kbi_y_trans_id==777] <- "Refuse to Answer"
table_three %>% tbl_summary(label = list( truesex~"Sex Assigned at Birth", kbi_y_trans_id~"Are you Transgender?")) %>% modify_header(label = "**1st Year Follow up**")