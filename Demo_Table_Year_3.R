...
table_one <- historya3 %>% select(Age,truesex,race_ethnicity,kbi_y_trans_id,demo_prnt_ed_v2_2yr_l, demo_prnt_ed_v2_l,eventname)
...
table_one %>% tbl_summary(label = list(Age ~"Patient Age", truesex~"Sex Assigned at Birth", kbi_y_trans_id~"Are you Transgender?")) %>% modify_header(label = "**3rd Year Follow up**")
